...
func (client *Client) ConnectAndWrite(ctx appengine.Context, resp *PushNotificationResponse, payload []byte) (err error) {
	...
	conn, err := socket.Dial(ctx, "tcp", client.Gateway)
	if err != nil {
		return err
	}
	defer conn.Close()
...
func (client *Client) ListenForFeedback(ctx appengine.Context) (err error) {
	var cert tls.Certificate
...
func (client *Client) Send(ctx appengine.Context, pn *PushNotification) (resp *PushNotificationResponse) {
	...
	err = client.ConnectAndWrite(ctx, resp, payload)
	if err != nil {
		resp.Success = false
		resp.Error = err
		return
